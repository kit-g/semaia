...
abstract class AbstractTextExport<T> {
  ...
  List<String> getColumnTitles(PlutoGridStateManager state) => visibleColumns(state).map((e) => e.title).toList();
  ...
  List<List<String?>> mapStateToListOfRows(PlutoGridStateManager state) {
    ...
    rowsToExport = state.refRows.filteredList.isNotEmpty ? state.refRows.filteredList : state.refRows.originalList;
